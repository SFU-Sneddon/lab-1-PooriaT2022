...
**Task** \* Duplicate the last line in this code chunk and modify it by replacing the single `>` with `>>`. What happens when you rerun it? it doubles the number of lines in the file. The single `>` overwrites the file where as the `>>` adds the first 25 lines of the head to the file again without overwriting it.

-   What happens if you change the output file name (after `>>`) to `~/MBB243/out/genotype_head.txt`? Why do you think that is? It changes the folder path that R will tries to save the `genotype_head.txt` file to `~/MBB243/out/genotype_head.txt` since there is no `out` folder in the `MBB243` folder bash will give us an error that it cannot find the `out` folder to save the `genotype_head.txt` file in it.
...
You should have a few lines of genotype information in a new file named `~/MBB243/genotype_head.txt`. Do you notice any genotypes in there that look "off"? Some genotypes are shown as `--` instead of showning homozygous and hetrozygous genotypes. This means that genotype for those genes are "not called" which means we are missing some of the data.

Assuming we wanted to sanity check this whole file for what genotypes exist, we can use a combination the `cut`, `uniq` and `sort` commands (uniq is dumb and only works properly if its input is sorted). The code chunk below does this but uses our smaller file as input for efficiency. The result should be all unique genotypes in that file. What did we do wrong? When we sort for uniq genotypes we found out that we have missing data since the `--` is showing up as a genotype. We can clear this if we wanted to.
...
#the cut -f 4 ~/MBB243/genotype_head.txt output is the cut out column 4 (genotypes)
cut -f 4 ~/MBB243/genotype_head.txt | sort 
#the cut -f 4 ~/MBB243/genotype_head.txt| sort sorts the cut column 4 (genotypes) in alphabetic order
cut -f 4 ~/MBB243/genotype_head.txt | sort | uniq
#the cut -f 4 ~/MBB243/genotype_head.txt | sort | uniq command only shows every uniqe genotype in column 4
```
...
The \>\> redirect operator keeps adding to the file the text "Hello terminal!" and date without replacing or overwritting the file. But If we used the \> it would overwrite the file so we would only get two lines of text if we typed in "echo"Hello terminal!" \> EMPTY.txt" and "echo \$(date) \> EMPTY.txt" no matter how many times we ran the code.
...
#both function to save files and changes. But the > overwrites the file where as the >> adds data to the file without replacing it.
...
```{bash, engine.opts='-l'}
echo $(samtools faidx ~/files/hg38.fa chr14:65102304-65102339 | grep -v \>) > ourfile.txt
```
